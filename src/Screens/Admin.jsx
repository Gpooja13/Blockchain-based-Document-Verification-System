    ...
    const fetchEvents = async () => {
      ...
      try {
        if (ownerAddress) {
          ...
          const pastDelEvents = await contract.getPastEvents(
            "ExporterDeleted",
            {
              filter: {},
              fromBlock: 0,
  ...
  return (
    <div>
      ...
      {userAddress === ownerAddress ? (
        ...
        <div className="flex items-center justify-center h-[90vh] text-2xl text-gray-700">
          <FiAlertCircle />
          <p className="ml-2">Not Authorized </p>
        </div>
      )}
    </div>
  );
